...
import 'package:fed_tms/features/documents/data/models/document.dart';
import 'package:fed_tms/features/documents/data/models/document.dart';
import 'package:fed_tms/features/documents/data/models/document.dart';
import 'package:fed_tms/features/documents/data/services/document_service.dart';
import 'package:fed_tms/features/documents/presentation/screens/documents_manager_screen.dart';
...
final documentsProvider =
    FutureProvider.family<List<Document>, DocumentFilters>(
        (ref, filters) async {
...
final driverDocumentsProvider =
    FutureProvider.family<List<Document>, String>((ref, driverId) async {
  final documentService = ref.watch(documentServiceProvider);
  return await documentService.getDriverDocuments(driverId);
});

// Load Documents Provider
final loadDocumentsProvider =
    FutureProvider.family<List<Document>, String>((ref, loadId) async {
  final documentService = ref.watch(documentServiceProvider);
...
class DocumentFilters {
  final String? search;
  final doc_model.DocumentType? type;
  final doc_model.DocumentStatus? status;
  final int page;
  final int limit;
  ...
  DocumentFilters copyWith({
    String? search,
    doc_model.DocumentType? type,
    doc_model.DocumentStatus? status,
    int? page,
    int? limit,
  }) {
...
class DocumentFiltersNotifier extends Notifier<DocumentFilters> {
  ...
  void setType(doc_model.DocumentType? type) {
...
final documentFiltersProvider =
    NotifierProvider<DocumentFiltersNotifier, DocumentFilters>(() {
