...
// Security, request marking, routing to Co-Driver
...
#[derive(Clone)]
struct MarketeerState {
    config: Arc<MarketeerConfig>,
    client: reqwest::Client,
}

#[derive(Debug, Clone, Deserialize)]
struct MarketeerConfig {
    listen_addr: String,
    listen_port: u16,
    codriver_url: String,
...
async fn load_config() -> Result<MarketeerConfig> {
    ...
    Ok(MarketeerConfig {
        listen_addr: std::env::var("MARKETEER_LISTEN_ADDR")
            .unwrap_or_else(|_| "0.0.0.0".to_string()),
        listen_port: std::env::var("MARKETEER_LISTEN_PORT")
            .unwrap_or_else(|_| "8000".to_string())
            .parse()
            .unwrap_or(8000),
        codriver_url: std::env::var("CODRIVER_URL")
...
async fn proxy_handler(
    ...
    // Add marking header if enabled
    let mut req_headers = headers.clone();
    if state.config.marking.enabled {
        let mark_id = format!("{}-{}", state.config.marking.prefix, Uuid::new_v4());
        req_headers.insert(
            state.config.marking.header_name.parse().unwrap(),
            mark_id.parse().unwrap(),
        );
    }
