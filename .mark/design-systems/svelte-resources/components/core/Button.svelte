<!-- src/lib/components/core/Button.svelte -->
<script lang="ts">
	...
	// Correctly typed optional function
	...
	function handleClick(event: MouseEvent) {
		if (!disabled && !loading) {
			onClick?.(event); // now TypeScript knows this is callable
		}
	}
</script>

{#if href}
	<a {href} class={classes} class:opacity-50={disabled} on:click={handleClick}>
		{#if loading}
...
{:else}
	<button {type} {disabled} class={classes} class:opacity-50={loading} on:click={handleClick}>
		{#if loading}
			<svg class="-ml-1 mr-2 h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
				<circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"
				></circle>
